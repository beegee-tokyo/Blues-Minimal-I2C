...
void setup()
{
	...
	Serial.println("RAKwireless RUI3 Blues Sensor Node");
	Serial.println("------------------------------------------------------");
	...
	// Register the custom AT commands for the Blues Notecard
	if (!init_blues_at())
	{
		MYLOG("SETUP", "Add custom Blues AT command fail");
	}

	// Initialize the Blues Notecard
	has_blues = init_blues();
	if (has_blues)
	{
		MYLOG("SETUP", "Found Blues Notecard");
	}
...
void sensor_handler(void *)
{
	...
	MYLOG("UPLINK", "Packet after adding BME values %d", g_solution_data.getSize());

	g_solution_data.addVoltage(LPP_CHANNEL_BATT, api.system.bat.get());
	MYLOG("UPLINK", "Packet after adding battery value %d", g_solution_data.getSize());
	...
	{ 
		...
		{
			MYLOG("UPLINK", "Not joined, send only over cellular");
			if (blues_send_payload(g_solution_data.getBuffer(), g_solution_data.getSize()))
			{
